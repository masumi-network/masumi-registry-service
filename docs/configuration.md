...
- **CORS_ALLOWED_ORIGINS**: A comma-separated allowlist of additional browser origins that may call the API cross-origin. For local admin UI development (`pnpm frontend:dev` on port 3001), include `http://localhost:3001`. Leave unset/empty in production if unused (no wildcard CORS).
